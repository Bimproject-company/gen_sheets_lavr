...
// Type: SheetGen.Commands.GenerateAssembliesOnly
...
namespace SheetGen.Commands
{
  ...
  public class GenerateAssembliesOnly : IExternalCommand
  {
      ...
      ElementSet elements)
    {
      ...
      foreach (Element element in GenerateAssembliesOnly.GetElementsFromSelection(commandData))
      {
        ...
        if (string.IsNullOrEmpty(parametersMap.get_Item("Описание").AsString()) && GenerateAssembliesOnly.PassesVolumeFilter(instance))
          builder.RegisterType<ArcStoneTransformation>().As<ITransformation>();
        else if (string.IsNullOrEmpty(parametersMap.get_Item("Описание").AsString()) && !GenerateAssembliesOnly.PassesVolumeFilter(instance))
